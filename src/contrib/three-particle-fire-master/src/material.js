...
export default function makeMaterialClass() {
	...
	return class Material {
		...
		constructor( parameters ) {
            ...
            // XAV: lower fading (flame, not fire):
            'return t;',
						//'float tt = t * t;',
						//'return tt * tt;',
            ...
            // XAV: make it transparent:
            'vec3 colorBase = texture2D( map, texCoord ).rgb;',
            'float opacity = vOpacity * smoothstep(0.2, 0.4, dot(colorBase, vec3(0.3333333)));',
            'gl_FragColor = vec4( colorBase * color, opacity );',

					'}'
				].join( '\n' ),

				blending   : THREE.AdditiveBlending,
				depthTest  : false, // XAV: disable depth test
				depthWrite : false,
				transparent: true,
        fog: false,
				// fog        : true
